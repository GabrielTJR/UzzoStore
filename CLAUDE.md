...
- **Expiração do não pago** (migração `0017`): pedido `online` nasce com `expires_at = agora + 60 min` e o **`pg_cron`** (job `expira-pedidos-nao-pagos`, a cada 10 min) marca `expired` + `canceled`. O relógio roda **dentro do Postgres de propósito** — cron na Vercel seria invocação paga disparando sozinha para sempre. WhatsApp nasce com `expires_at` nulo: lá o pagamento é combinado por fora e pode levar dias.
