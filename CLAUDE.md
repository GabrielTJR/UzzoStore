...
As categorias do menu/filtro vêm do **banco** (`getCategories`, tabela `categories` kind `setor`) — geridas em `/admin/categorias`; `src/lib/categories.ts` guarda só o helper puro `categorySlug`. A página `/produtos` filtra por **categoria**, por **cor** (multi-seleção, **facetada** — só as cores existentes na categoria/promoção atual; cores selecionadas fora da base são ignoradas) e por **"Em promoção"** (`?promo=1` — produtos com `promo_price` cadastrado), tudo **cumulativo** via query string (`?categoria=<slug>&cores=Nome1,Nome2&promo=1`) e por `<Link>` (server-side; nenhuma cor selecionada = todas; clicar alterna). O carrinho é **client-side** (`src/lib/cart-store.ts`, Zustand + localStorage); o `CartItem` carrega `color` + `size` (a chave de dedupe continua sendo `variantId`, já que cada cor×tamanho é uma variante distinta). O checkout monta uma **mensagem de pedido no WhatsApp** (`/sacola`) que inclui a cor — ainda sem pagamento online.
