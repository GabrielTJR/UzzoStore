...
## Data model (see `supabase/migrations/20260725163425_0001_initial_schema.sql` and `..._0004_color_factor_global_colors_product_price.sql`)
...
- **Site-owned** (`colors`, `product_colors`, `product_content` = slug/SEO, `customers`, `carts`, `cart_items`, `reservations`, `orders`, `order_items`, `payments`, `sync_state`).

Key facts (updated in migration `0004` — the "Cor" feature):
- The **sellable unit is the variant** (`product_variants`), which is a **(color × size) grade**. Each variant belongs to a **`product_colors`** row via `product_color_id` (and keeps the free-text `color` = the color's name, for Microvix-mirror compatibility). Unique index on `(product_color_id, coalesce(size,''))`.
- **`colors`** is a **global color registry** (site-owned): `name` (unique) + optional `hex` swatch. Standardized names make cross-catalog color filtering possible.
- **`product_colors`** ties a global color to a product and owns that color's **`gallery`** (JSON array of Storage URLs). **Photos are per-color**, not per-product.
- **Price is GLOBAL per product**: `products.price` + `products.promo_price`. Effective price = `promo_price ?? price`. The per-variant **`prices` table is DORMANT** — kept for the eventual Microvix path but **not read by the site anymore**; do not add per-variant/per-color pricing to the UI.
- **Stock is per-variant** (`stock_cache`), so stock is naturally per (color, size).
- `product_content.gallery` is **legacy** (kept, not read by the storefront/admin after `0004`). A product only appears in the storefront when `active_ecommerce = true`.
...
Server Components fetch via `src/lib/products.ts` (`getProducts`, `getProductBySlug`). Nested Supabase embeds are cast to local `Row` types via `as unknown as` — deliberate, to avoid brittle generated-type inference on deep selects. `getProductBySlug` returns `colors` (each with its own `gallery` + size `variants`) and the product-level price. The product page is driven by a single client component **`src/components/product-view.tsx`**: it holds the selected color + size, **swaps the gallery and available sizes/stock when the color changes**, and is the only write path into the cart. The category menu is a **static list in `src/lib/categories.ts`** whose names must match the DB category names. Cart is **client-side** (`src/lib/cart-store.ts`, Zustand + localStorage); `CartItem` carries `color` + `size` (dedup key is still `variantId`, since each color×size is a distinct variant). Checkout builds a **WhatsApp order message** (`/sacola`) that includes the color — no online payment yet.
...
- **Pages**: `/admin` (products), `/admin/produtos/novo` + `/admin/produtos/[id]` (CRUD), `/admin/cores` (**global color registry** — create/rename/recolor/delete; a color in use can't be deleted, FK `on delete restrict`), `/admin/logs` (audit), `/admin/equipe` (manage admins — add/remove owner-only), `/admin/conta` (own name/password).
- **Color feature** (migration `0004`): the product edit page is organized **by color** — each `product_colors` row has its own photo grid + a size/stock grid (`VariantForm`, size + qty, **no price**). Global price + promo live on the `ProductInfoForm`. Add a color via `addProductColorAction` (pick from `colors` or create a new global color inline); remove via `removeProductColorAction` (cascades its variants/stock and deletes its Storage photos). New-product form (`createProductAction`) takes name/category/price/promo/sizes + one-or-more colors (existing `colorIds` and/or comma `newColors`) and builds the full color×size grade with stock 0.
...
- **Image uploads go DIRECT browser → Storage**, never through a Server Action body (Next caps it at 1 MB, Vercel at 4.5 MB — multiple phone photos blow past both and crash the page). Flow (`src/lib/upload-photos.ts`): `createUploadUrlsAction` (admin-gated) mints **signed upload URLs** (folder = the `product_colors.id`), the browser `uploadToSignedUrl`s each file, then `commitPhotosAction(productColorId, paths)` appends to that color's `product_colors.gallery` — the server rebuilds the public URL from the path, never trusting a client URL. `AddPhotosForm` (per color) is hand-rolled `onSubmit` (not `useActionState`) because the upload must run before the mutation. `removePhotoAction` takes `productColorId` + `url`. No Storage RLS policy needed (signed URLs self-authorize). (New-product no longer uploads a photo — photos are added per color on the edit page.)
