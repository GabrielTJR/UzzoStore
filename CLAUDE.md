...
O menu de categorias é uma **lista estática em `src/lib/categories.ts`** cujos nomes devem bater com os nomes das categorias no banco. O carrinho é **client-side** (`src/lib/cart-store.ts`, Zustand + localStorage); o `CartItem` carrega `color` + `size` (a chave de dedupe continua sendo `variantId`, já que cada cor×tamanho é uma variante distinta). O checkout monta uma **mensagem de pedido no WhatsApp** (`/sacola`) que inclui a cor — ainda sem pagamento online.
...
- **Páginas**: `/admin` (produtos), `/admin/produtos/novo` + `/admin/produtos/[id]` (CRUD), `/admin/cores` (**cadastro global de cores** — criar/renomear/recolorir/excluir; cor em uso não pode ser excluída, FK `on delete restrict`), `/admin/logs` (auditoria), `/admin/equipe` (gerenciar admins — adicionar/remover só-owner), `/admin/conta` (próprio nome/senha).
